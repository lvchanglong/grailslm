            ...
            <table>
                ...
                <g:each in="${articleList}" status="i" var="article">
                    <tr class="${(i % 2 == 0)?"even":"odd"}">
                        <td><g:link controller="article" action="show" id="${article.id}">${article.title}</g:link></td>
                        <td>${article.zindex}</td>
                        <td>${article.grouping}</td>
                        <td><img src="${createLink(controller: "article", action: "preview", id: article.id)}" style="max-width:25px;max-height:25px;"/></td>
                        <td><g:link controller="article" action="download" id="${article.id}">${article.dataname}</g:link></td>
